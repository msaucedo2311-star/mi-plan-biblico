...
function profile() {
  ...
  <section><h2>Servicio seguro de IA</h2><label>URL del backend/proxy<input name="apiBase" type="url" placeholder="https://api.tudominio.com" value="${esc(state.settings.apiBase)}"></label><p class="hint">Las claves privadas viven únicamente en ese backend, nunca en esta app.</p></section><button class="primary">Guardar ajustes</button></form>
...
function bind() {
  ...
  app.querySelector('[data-action="notifications"]')?.addEventListener('click',async()=>{if(!('Notification'in window))return toast('Este navegador no admite notificaciones.');const p=await Notification.requestPermission();toast(p==='granted'?'Notificaciones autorizadas. Consulta el README para recordatorios en segundo plano.':'Permiso no concedido.');});
